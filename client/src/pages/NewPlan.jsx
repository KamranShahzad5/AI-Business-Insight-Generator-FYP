  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass border border-brand-500/30 text-brand-300 text-xs font-semibold mb-4">
        ...
        {loading ? (
          ...
          >
            ...
            <motion.button
              ...
              className="w-full btn-primary justify-center py-4 text-sm shadow-glow-md disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >
              <Sparkles size={18} />
            ...
            <p className="text-center text-xs text-slate-600">
              Takes 3–5 seconds · Powered by simulated AI engine
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
function LoadingScreen({ loadStep }) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className="card border border-brand-500/20 py-12 text-center"
    >
      <motion.div
        animate={{ rotate: 360 }}
        transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
        className="w-20 h-20 rounded-2xl bg-gradient-to-br from-brand-500 to-accent-500 flex items-center justify-center mx-auto mb-6 shadow-glow-md"
      >
      ...
      <p className="text-sm text-slate-400 mb-8">Analyzing your idea and generating insights</p>
      ...
      <div className="space-y-3 max-w-sm mx-auto text-left">
        {loadingSteps.map((step, i) => {
          ...
          return (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${isDone ? 'bg-emerald-500/20' : isActive ? 'bg-brand-500/20' : 'bg-white/5'}`}>
                {isDone
              ...
              <span className={`text-sm ${isDone ? 'text-emerald-400 line-through' : isActive ? 'text-slate-100' : 'text-slate-600'}`}>
                {step.label}
              </span>
            </motion.div>
          );
