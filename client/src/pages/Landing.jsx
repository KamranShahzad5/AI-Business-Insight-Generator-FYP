...
import {
  ...
  CheckCircle, Star, ChevronRight, Brain, Rocket, TrendingUp,
} from 'lucide-react';
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-surface-950 text-slate-100 overflow-x-hidden">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-16 h-16 bg-surface-950/80 backdrop-blur-xl border-b border-white/5">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-24 pb-20 px-6 text-center">
        ...
        <div className="absolute top-1/2 left-1/4 w-[300px] h-[300px] bg-accent-600/8 rounded-full blur-3xl pointer-events-none" />
        ...
        >
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass border border-brand-500/30 text-brand-300 text-xs font-semibold mb-8"
          >
            <Sparkles size={12} />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="btn-primary text-base py-3.5 px-8 shadow-glow-md"
            >
              <Rocket size={18} />
              Start for Free
              <ArrowRight size={16} />
        ...
        >
          <div className="glass rounded-2xl border border-white/10 p-4 shadow-glow-lg">
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              {['Business Plan', 'Tasks', 'Costs', 'Risks'].map((t, i) => (
                <div key={i} className={`py-2 rounded-lg text-xs font-semibold text-center transition-all ${i === 0 ? 'bg-brand-500/20 text-brand-300 border border-brand-500/30' : 'bg-white/3 text-slate-500'}`}>
                  {t}
                </div>
              ))}
            </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feat, i) => (
              ...
              >
                <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${feat.gradient} flex items-center justify-center mb-4 shadow-glow-sm group-hover:shadow-glow-md transition-all duration-300`}>
                  <feat.icon size={20} className="text-white" />
                </div>
                <h3 className="font-display font-semibold text-slate-100 mb-2">{feat.title}</h3>
      ...
      {/* How it works */}
      <section id="how-it-works" className="py-24 px-6 bg-surface-900/30">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-display font-bold mb-4">
              <span className="text-gradient">3 Simple Steps</span> to Your Business Plan
            </h2>
            <p className="text-slate-400 mb-16">No consulting fees. No weeks of research. Just results.</p>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              >
                ...
                {i < 2 && (
                  <div className="hidden md:flex absolute top-1/2 -right-4 z-10 text-slate-600">
      ...
      <section id="testimonials" className="py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-display font-bold mb-3">Loved by Entrepreneurs</h2>
            <p className="text-slate-400">Trusted by founders, analysts, and innovators worldwide.</p>
          </motion.div>
